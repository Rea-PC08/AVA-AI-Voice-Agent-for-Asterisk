...
- **Native Fish Audio modular TTS provider** ([#653](https://github.com/hkjarral/AVA-AI-Voice-Agent-for-Asterisk/pull/653), contributed by [@Rea-PC08](https://github.com/Rea-PC08)): streams Fish Audio PCM progressively at the negotiated call rate, supports provider-scoped managed API keys and Admin UI configuration, enforces encrypted remote endpoints, and bounds connection and stalled-stream waits without cutting off a healthy long synthesis. **Test Connection** performs a short real synthesis so model entitlement, account-credit, and empty-audio failures are caught before a call. The bundled loopback mock covers credential, streaming, timeout, cancellation, and error paths without requiring an account.
...
- **Opt-in Asterisk Media WebSocket transport**: `audio_transport: websocket` plus a new `websocket_media` block (listener bind/advertise host, port, path, fallback wire format, control format, source allowlist, auth, TLS). Asterisk's `chan_websocket` opens a per-call media connection to the AI Engine listener; the media password is supplied only through the env var named by `websocket_media.auth.password_env` (default `ASTERISK_MEDIA_WS_PASSWORD`), never in YAML. JSON controls require Asterisk 20.18+, 22.8+, or 23.2+; an experimental `control_format: auto`/`plain` path covers exactly 20.17.0; other versions fail closed. The Admin UI Transport page adds a WebSocket editor with version/module/secret-presence checks and a generated `websocket_client.conf` stanza, and `agent check`/`preflight.sh` validate the prerequisites. Only OpenAI Realtime with `ulaw` on Asterisk 22.10.1/FreePBX 17 has a live smoke pass; every other provider/codec/topology combination remains unqualified. See `docs/WebSocket-Transport.md`.
- **Opt-in call metadata persistence and call-local correction** ([#625](https://github.com/hkjarral/AVA-AI-Voice-Agent-for-Asterisk/issues/625), [#627](https://github.com/hkjarral/AVA-AI-Voice-Agent-for-Asterisk/issues/627)): individual pre-call HTTP lookup outputs can be selected for bounded, non-authoritative persistence in Call History, exposed to prompts and post-call webhooks, filtered by exact field/value, and included in CSV/JSON exports. Fields explicitly marked correctable can be changed during the active call only when the Agent is separately granted `update_call_metadata`; corrections preserve the original pre-call snapshot and never write back to the CRM or mutate caller identity, routing, consent/DNC, transfer, disposition, provider, or external-dialer state. The Admin UI adds per-output persistence/correction controls plus metadata provenance in Call History. Across the metadata and troubleshooting features, existing databases receive three additive SQLite columns automatically: `call_metadata`, `call_metadata_updates`, and `diagnostics_snapshot`. Existing records remain valid with empty metadata and no settings snapshot, and disabling the metadata policy stops future persistence without removing historical values.

### Fixed

- **Admin UI remains responsive when Docker log reads stall** ([#648](https://github.com/hkjarral/AVA-AI-Voice-Agent-for-Asterisk/issues/648)): Docker SDK log retrieval now runs outside FastAPI's asyncio event loop with a bounded daemon timeout, while the Logs page serializes refreshes and backs off after failures. Setup detection no longer reruns on every route change or turns one transient probe failure into a persistent full-screen "Backend unavailable" state.
- **Non-finite numeric configuration is rejected before persistence** ([#648](https://github.com/hkjarral/AVA-AI-Voice-Agent-for-Asterisk/issues/648)): Admin UI saves now reject YAML/JavaScript `NaN` and infinite values with their exact config paths, structured reads diagnose older poisoned overrides without an ASGI serialization traceback, and Google Live numeric controls no longer emit `.nan` when cleared.
- **WebSocket review hardening**: recover a missing AI Engine container through Compose after validating its saved media credential; handle scalar allowlists and cleared numeric fields in the Transport UI; recognize commented transport selections in preflight; compare equivalent IPv6 and IPv4-mapped peer addresses consistently. Setup-abort cleanup failures now include diagnostic context, and the trusted-loopback authentication opt-out is documented explicitly.
- **Pipeline farewell/transfer file playback with PCM TTS profiles** (`src/engine.py`): pipeline farewells, deferred-transfer announcements, and the file-playback fallback wrote raw TTS bytes to a `.ulaw` file. With a 16 kHz PCM audio profile (e.g. `wideband_pcm_16k` with local TTS) Asterisk played PCM samples as mu-law: garbled audio, four times too long, and a delayed hangup. PCM TTS output is now resampled to 8 kHz and mu-law encoded before file playback.
- **Admin UI transport apply and status**: hot-reload of restart-only transport keys is reported as restart-required instead of a failure; Restart AI Engine no longer depends on YAML parseability; Asterisk status resolves the selected transport YAML-first like the engine; preflight no longer reports modules of unselected transports as failures.
- **Gemini post-call summaries can be configured without raw YAML** ([#639](https://github.com/hkjarral/AVA-AI-Voice-Agent-for-Asterisk/issues/639), follow-up to [#618](https://github.com/hkjarral/AVA-AI-Voice-Agent-for-Asterisk/issues/618)): **Providers → Add Provider Templates** now includes a disabled `google_llm` modular template wired to the existing native Gemini adapter and provider-scoped `GOOGLE_API_KEY` flow. Creating it preserves existing providers; operators can configure and enable it before selecting it for post-call summaries or pipelines.
